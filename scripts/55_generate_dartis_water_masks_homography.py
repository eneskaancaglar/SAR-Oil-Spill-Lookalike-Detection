...
OUTPUT_DIR = (
    ROOT
    / "outputs"
    / "v06_dartis_water_masks_homography"
)
...
REPORT_PATH = (
    ROOT
    / "reports"
    / "v06_dartis_water_masks_homography.md"
)
...
def solve_projective_homography(
    source_points: np.ndarray,
    target_points: np.ndarray,
) -> np.ndarray:
    if source_points.shape != (4, 2):
        raise ValueError(
            "source_points ?ekli (4, 2) olmal?d?r."
        )

    if target_points.shape != (4, 2):
        raise ValueError(
            "target_points ?ekli (4, 2) olmal?d?r."
        )
...
def create_geospatial_land_mask(
    *,
    ...
    upper_left = (
        float(row["patch_ul_lon"]),
        float(row["patch_ul_lat"]),
    )

    upper_right = (
        float(row["patch_ur_lon"]),
        float(row["patch_ur_lat"]),
    )

    bottom_right = (
        float(row["patch_br_lon"]),
        float(row["patch_br_lat"]),
    )

    bottom_left = (
        float(row["patch_bl_lon"]),
        float(row["patch_bl_lat"]),
    )

    # Dikd?rtgen bbox yerine ger?ek SAR g?r?nt? d?rtgeni.
    ...
    patch_polygon = safe_geometry(
        patch_polygon
    )

    if patch_polygon is None:
        raise RuntimeError(
            "Ge?erli g?r?nt? co?rafi d?rtgeni olu?turulamad?."
        )
    ...
    for geometry in candidate_geometries:
        ...
        try:
            # Kara poligonunu eksen hizal? kutuyla de?il,
            # g?r?nt?n?n ger?ek d?rtgeniyle kes.
    ...
    # Enlem-boylam de?erleri birbirine ?ok yak?n oldu?u i?in
    # say?sal kararl?l?k amac?yla normalize ediyoruz.
    geographic_center = (
        geographic_points.mean(
            axis=0
        )
    )
    ...
    if geographic_span <= 1e-12:
        raise RuntimeError(
            "G?r?nt? k??e koordinatlar? ??km?? durumda."
        )

    normalized_geographic_points = (
        geographic_points
        - geographic_center
    ) / geographic_span
    ...
    homography = solve_projective_homography(
        normalized_geographic_points,
        pixel_points,
    )
